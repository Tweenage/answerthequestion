...
export const batch2NonVerbalReasoningQuestions: Question[] = [
...

  {
    ...
    subject: 'reasoning',
    difficulty: 1,
    questionText: 'Look at the sequence: 1, 2, 4, 7, 11, ___. The gap between each pair of numbers increases by 1 each time. What number comes next?',
    questionTokens: ['Look', 'at', 'the', 'sequence:', '1,', '2,', '4,', '7,', '11,', '___.', 'The', 'gap', 'between', 'each', 'pair', 'of', 'numbers', 'increases', 'by', '1', 'each', 'time.', 'What', 'number', 'comes', 'next?'],
    keyWordIndices: [3, 4, 5, 6, 7, 8, 9, 11, 17, 19, 22, 23, 25],
    options: [
      { text: '14', isEliminatable: true, eliminationReason: '14 adds only 3 to 11, copying the previous gap. But the gap increases each time: +1, +2, +3, +4, so the next gap is +5.' },
      { text: '16', isEliminatable: false },
      { text: '15', isEliminatable: true, eliminationReason: '15 adds 4 to 11. The gap from 7 to 11 was already +4, so the next gap must be +5, giving 16.' },
      { text: '12', isEliminatable: true, eliminationReason: '12 adds only 1 to 11, using the first gap. The gaps grow: +1, +2, +3, +4, +5.' },
      { text: '17', isEliminatable: true, eliminationReason: '17 adds 6 to 11. The next gap should be +5, not +6. The gaps are: 1, 2, 3, 4, 5.' },
    ],
    correctOptionIndex: 1,
    explanation: 'Step 1: Find the gaps: 2-1=1, 4-2=2, 7-4=3, 11-7=4. Step 2: The gaps increase by 1, so the next gap is 5. Step 3: 11+5=16.',
  ...
  },
  {
    ...
    subject: 'reasoning',
    difficulty: 1,
    questionText: 'In a code, circle = 3, triangle = 5, and square = 7. A "combo" is made of two different shapes added together. Which combo has the same value as a square plus a circle minus a triangle?',
    questionTokens: ['In', 'a', 'code,', 'circle', '=', '3,', 'triangle', '=', '5,', 'and', 'square', '=', '7.', 'A', '"combo"', 'is', 'made', 'of', 'two', 'different', 'shapes', 'added', 'together.', 'Which', 'combo', 'has', 'the', 'same', 'value', 'as', 'a', 'square', 'plus', 'a', 'circle', 'minus', 'a', 'triangle?'],
    keyWordIndices: [3, 5, 6, 8, 10, 12, 23, 24, 27, 28, 31, 32, 34, 35, 37],
    options: [
      { text: 'circle + triangle (= 8)', isEliminatable: true, eliminationReason: 'circle + triangle = 3 + 5 = 8, but the target is square + circle - triangle = 7 + 3 - 5 = 5, not 8.' },
      { text: 'circle + circle (= 6)', isEliminatable: true, eliminationReason: 'circle + circle = 6, which is close but not 5. Also, the combo must use two different shapes.' },
      { text: 'triangle + square (= 12)', isEliminatable: true, eliminationReason: 'triangle + square = 5 + 7 = 12. The target value is 5, not 12.' },
      { text: 'No combo equals 5', isEliminatable: false },
      { text: 'circle + square (= 10)', isEliminatable: true, eliminationReason: 'circle + square = 3 + 7 = 10. The target value is 5, not 10.' },
    ],
    correctOptionIndex: 3,
    explanation: 'Step 1: Calculate the target: square + circle - triangle = 7 + 3 - 5 = 5. Step 2: Check all combos of two different shapes: 3+5=8, 3+7=10, 5+7=12. None equal 5, so no combo matches.',
    category: 'logic-code',
    trickType: 'two-step',
  },
...

  {
    ...
    options: [
      ...
      { text: 'North-West', isEliminatable: true, eliminationReason: 'North-West is a diagonal direction. Standard 90-degree turns only reach the four main compass points.' },
    ],
  ...
  },
  {
    ...
    options: [
      ...
      { text: '13', isEliminatable: true, eliminationReason: '13 adds 3 to 10. It\'s easy to see Row 2 adds 3 each time and apply the same gap, but Row 3 multiplies 5 by 1, 2, 3.' },
    ],
    correctOptionIndex: 1,
  ...
  },
  {
    ...
    options: [
      ...
      { text: 'Set P only (because 3 comes first)', isEliminatable: true, eliminationReason: 'The order the rules are listed does not matter. 6 meets BOTH conditions, so it belongs in both sets.' },
    ],
...

  {
    ...
    options: [
      ...
      { text: '12', isEliminatable: true, eliminationReason: '12 adds 4 to 8. It\'s easy to see the gaps increasing (1, 1, 2, 3) and guess the next gap is 4, but you add the two previous numbers: 5 + 8 = 13.' },
    ],
    correctOptionIndex: 2,
  ...
  },
  {
    ...
    options: [
      ...
      { text: '5', isEliminatable: true, eliminationReason: '5 would mean only 1 face was painted. It\'s easy to think "share one corner" means only one face, but three faces meet at each corner of a cube.' },
    ],
    correctOptionIndex: 1,
  ...
  },
  {
    ...
    options: [
      ...
      { text: '15', isEliminatable: true, eliminationReason: '15 might come from adding 5+4+3+2+1. But the correct count is 4+3+2+1 = 10. It\'s easy to start with 5 instead of 4.' },
    ],
    correctOptionIndex: 1,
  ...
  },
  {
    ...
    options: [
      ...
      { text: '14', isEliminatable: true, eliminationReason: '14 could come from using pentagon (5) instead of hexagon (6): 5+5+4. It\'s easy to mix up shape values.' },
    ],
    correctOptionIndex: 1,
  ...
  },
  {
    ...
    options: [
      ...
      { text: '17', isEliminatable: true, eliminationReason: '17 might come from adding 10+7 (mixing middle and bottom rows). The top brick only adds the two middle bricks: 10+9=19.' },
    ],
    correctOptionIndex: 1,
